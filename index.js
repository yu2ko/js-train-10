...
function customAssign(...objects) {
  ...
  if (objects.length < 2) {
    ...
    let fusionObjects = Object.assign(...objects);
    for (const value in fusionObjects) {
      if (typeof fusionObjects[value] === "number") {
        fusionObjects[value] = fusionObjects[value].toString();
      }
    }
...
function checkProperty(obj, prop) {
  ...
  if (typeof obj !== "object") {
    ...
    const checkProperty = obj.hasOwnProperty(prop);
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  if (typeof obj1 !== "object" || typeof obj2 !== "object") {
    ...
    if (Object.isFrozen(newObj)) {
      ...
      return "Error: об'єкт не було заморожено";
    }
  }
}
...
function filterObjectsByKey(arr, key) {
  ...
  if (!Array.isArray(arr)) {
    ...
    const newArray = arr.filter((obj) => obj.hasOwnProperty(key));
    return newArray;
  }
}
...
function getObjectValuesSum(obj) {
  ...
  return values.reduce((accum, curValue) => accum + curValue, 0);
}
